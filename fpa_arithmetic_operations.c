...
return;
  ...
  void Mult(double xh1, double xl1, double xh2, double xl2,double *t, double *z) {
    ...
    xl3 +=  xh_1*xl_2;
    xl3 +=  xl_1*xh_2;
    xl3 +=  xl_1*xl_2+ xl1*xh2 + xh1*xl2; // test it
   // xl3 +=  xl_1*xl_2; // finish like this in the Müller's book
    F2Sum(xh3, xl3, t, z);
    return;
    } 

    // Dekker's Two Product
    void Mult2(double xh1, double xl1, double xh2, double xl2,double *t, double *z) {
    double p, q, r;
    double xh_1, xl_1, xh_2, xl_2, xh3, xl3;
    VeltkampSplit(xh1, &xh_1, &xl_1); 
    VeltkampSplit(xh2, &xh_2, &xl_2); 
...
return;
   ...
   void scalar_product(double xh[3], double xl[3], double *xh3, double *xl3) {  // the square of the vector itself
     ...
     Mult(y0, y1, t4, t5, xh3, xl3); // k^2*sqrt(k)    
     } 
...
// SUMMATION ACCURACY TEST
    ...
    Mult2(prod_h, prod_l, c3, 0, &prod_h, &prod_l);
  ...
  // DIVISION ACCURACY TEST2
   ...
   printf(" A-double= %.20Le A-pairwise= %.20Le \n", A-B1, A-B2); 
